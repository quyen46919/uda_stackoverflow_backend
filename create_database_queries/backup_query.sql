...
CREATE SCHEMA IF NOT EXISTS `uda_stackoverflow` DEFAULT CHARACTER SET utf8 ;
...
CREATE TABLE IF NOT EXISTS `uda_stackoverflow`.`uda_user_settings` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;

CREATE TABLE IF NOT EXISTS `uda_stackoverflow`.`uda_settings` (
...
CREATE TABLE IF NOT EXISTS `uda_stackoverflow`.`uda_member_of_team` (
  ...
  `role` ENUM('Trưởng nhóm', 'Phó nhóm', 'Thành viên') NULL DEFAULT NULL,
  `created_at` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `uda_stackoverflow`.`uda_user_settings` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_uda_user_settings_uda_users1_idx` (`user_id` ASC),
  INDEX `fk_uda_user_settings_uda_settings1_idx` (`setting_id` ASC),
  CONSTRAINT `fk_uda_user_settings_uda_users1`
    FOREIGN KEY (`user_id`)
    REFERENCES `uda_stackoverflow`.`uda_users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_uda_user_settings_uda_settings1`
    FOREIGN KEY (`setting_id`)
    REFERENCES `uda_stackoverflow`.`uda_settings` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
